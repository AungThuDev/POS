...
@section('content')
    <div class="container-fluid">
        ...
        <div class="p-5">
            <form action="{{ route('recipe.update',$recipe->id) }}" method="POST" class="form-group" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
                <label class="form-label" for="name">Name</label>
                <input name="name" id="name" type="text" class="form-control" value="{{$recipe->name}}">
                @error('name')
                    <span class="text-danger d-block">{{ $message }}</span>
                @enderror
                <label for="price" class="form-label mt-2">Price</label>
                <input name="price" id="price" type="text" class="form-control" value="{{$recipe->price}}">
                @error('price')
                    <span class="text-danger d-block">{{ $message }}</span>
                @enderror
                ...
                <select name="kitchen_id" class="form-select">
                    @if (!empty($kitchens))
                        @foreach ($kitchens as $kitchen)
                            <option value="{{ $kitchen->id }}" {{ $kitchen->id === $recipe->kitchen_id ? 'selected' : '' }}>{{ $kitchen->name }}</option>
                        @endforeach
                    @endif
                </select>
                @error('kitchen_id')
                ...
                <div class="mt-2 d-flex align-items-center justify-content-center flex-wrap">
                    ...
                    <div class="ms-4">
                        <img class="rounded border border-1 border-black shadow-sm" id="uploadImage" style="width: 250px;"
                            src="{{$recipe->getImageSrc()}}">
                    </div>
                </div>
                @error('image')
                    <span class="text-danger d-block">{{ $message }}</span>
